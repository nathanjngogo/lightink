...
contextBridge.exposeInMainWorld('mdr', {
  ...
  shelfData: () => ipcRenderer.invoke('shelf:data'),
  shelfFolderAdd: (name) => ipcRenderer.invoke('shelf:folder-add', name),
  shelfFolderRename: (id, name) => ipcRenderer.invoke('shelf:folder-rename', id, name),
  shelfFolderRemove: (id) => ipcRenderer.invoke('shelf:folder-remove', id),
  shelfFileAdd: (id, p) => ipcRenderer.invoke('shelf:file-add', id, p),
  shelfFileRemove: (id, p) => ipcRenderer.invoke('shelf:file-remove', id, p),
  shelfReadFiles: (paths) => ipcRenderer.invoke('shelf:read-files', paths),
  docCreate: (name, dir) => ipcRenderer.invoke('doc:create', name, dir),
